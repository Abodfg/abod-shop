...
async def create_admin_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("📣 إرسال إعلان للمستخدمين", callback_data="broadcast_message")],
...
🎉 مرحباً بالعميل الجديد في عائلة Abod Shop الرقمية! ✨"""
    ...
    if start_param:
        ...
        if start_param.startswith("cat_"):
            # رابط مباشر لفئة - عرض جميع فئات المنتج من الإعلان
            category_id = start_param.replace("cat_", "")
            logging.info(f"Processing category deep link: {category_id}, ad_id: {ad_id}")
            
            # تسجيل مشاهدة الإعلان
            if ad_id:
                await track_ad_interaction(ad_id, telegram_id, "view", "channel")
                
                # CRITICAL: الحصول على المنتج من الإعلان وليس من الفئة!
                ad = await db.channel_ads.find_one({"ad_id": ad_id})
                
                if ad and ad.get('product_id'):
                    # عرض فئات المنتج الموجود في الإعلان
                    product = await db.products.find_one({"id": ad['product_id'], "is_active": True})
...
💎 *الباقات المتاحة:*"""
                            ...
                            logging.info(f"Showed {len(categories)} categories for product {product['name']} from ad {ad_id}")
                            return
                        else:
                    ...
                    logging.warning(f"Ad {ad_id} not found or has no product_id")
                    # Fallback: استخدام category_id
                    ...
                    if category:
                        ...
                        if product:
                            await send_user_message(telegram_id, f"⚠️ تم فتح المنتج: {product['name']}")
                            # عرض المنتج بشكل عادي
                            await handle_user_product_selection(telegram_id, product['id'])
                            return
            else:
                # بدون ad_id - استخدام category_id عادي
                category = await db.categories.find_one({"id": category_id, "is_active": True})
...
_اضغط على الرقم أعلاه لنسخه تلقائياً_"""
            ...
            # إرسال إشعار popup (لا حاجة له - الرسالة واضحة)
        else:
            await send_user_message(telegram_id, "❌ طريقة الدفع غير موجودة")
...
async def handle_browse_products(telegram_id: int):
    ...
    # رابط التطبيق الجديد
    app_url = f"{BASE_URL}/api/app?user_id={telegram_id}"
...
async def handle_admin_message(message):
    ...
    if not await is_admin_authorized(telegram_id):
...
إذا كنت تريد استخدام الخدمات، يرجى استخدام بوت المستخدمين."""
        ...
        logging.critical(f"🚨 SECURITY ALERT: Unauthorized admin access attempt by {telegram_id}")
        return
...
async def setup_bot_ui():
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # تعيين وصف البوت (محسّن ومختصر)
            description_response = await client.post(
                f"https://api.telegram.org/bot{USER_BOT_TOKEN}/setMyDescription",
                json={
                    "description": """🎮 شحن ببجي | فري فاير | كول أوف ديوتي | فورتنايت
🎁 بطاقات جوجل بلاي | ايتونز | بلايستيشن | اكس بوكس

⚡ توصيل فوري - أسعار منافسة - دعم 24/7
متجر Abod Shop - أفضل متجر رقمي للشحن
