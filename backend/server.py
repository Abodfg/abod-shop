...
🎉 مرحباً بالعميل الجديد في عائلة Abod Store السحرية! ✨"""
    ...
    welcome_text = f"""🌟✨ *أهلاً وسهلاً في عالم Abod Store السحري!* ✨🌟

🎭 مرحباً {first_name or 'المسافر الكوني'}! 
...
👇✨ *ادخل إلى عالمك السحري الآن!* ✨👇"""
    
    # إنشاء زر Web App مخصص مع تصميم سحري
    web_app_url = f"https://digicardbot.preview.emergentagent.com/api/app?user_id={telegram_id}"
    keyboard = [
...
{"يمكنك الآن إضافة أكواد لهذه الفئة." if category.delivery_type == "code" else "هذه الفئة تتطلب تنفيذ يدوي للطلبات."}"""
    ...
    # Handle stars addition
    elif session.state == "adding_stars":
        step = session.data.get("step", "user_id")
        ...
        if step == "user_id":
            try:
                ...
                if user:
                    ...
                    current_stars = user.get('balance_stars', 0)
                    await send_admin_message(telegram_id, 
                        f"⭐ *إضافة نجوم للمستخدم:* {user.get('first_name', 'غير معروف')}\n"
                        f"🆔 المعرف: `{user_telegram_id}`\n"
                        f"💫 الرصيد الحالي: {current_stars} نجمة\n\n"
                        f"💡 أدخل عدد النجوم المراد إضافتها:")
                else:
                    await send_admin_message(telegram_id, "❌ المستخدم غير موجود. يرجى إدخال معرف صحيح:")
            except ValueError:
                await send_admin_message(telegram_id, "❌ يرجى إدخال رقم صحيح:")
            ...
            try:
                ...
                if stars_amount <= 0:
                    await send_admin_message(telegram_id, "❌ يرجى إدخال عدد نجوم أكبر من صفر:")
                    return
                ...
                new_balance = user.get('balance_stars', 0) + stars_amount
                await db.users.update_one(
                    {"telegram_id": user_telegram_id},
                    {"$set": {"balance_stars": new_balance}}
                )
                
                # Record the transaction
                await record_stars_transaction(
                    user['id'], user_telegram_id, "admin_add", 
                    stars_amount, "admin_add"
                )
                
                # Send notification to user
                usd_equivalent = stars_to_usd(stars_amount)
                await send_user_message(
                    user_telegram_id,
                    f"⭐ *تم شحن محفظة النجوم بنجاح!*\n\n"
                    f"💫 النجوم المضافة: {stars_amount} نجمة\n"
                    f"💰 المعادل: ${usd_equivalent:.2f}\n"
                    f"🌟 الرصيد الجديد: {new_balance} نجمة\n\n"
                    f"🎉 شكراً لك! يمكنك الآن التسوق بحرية"
                )
                
                await clear_session(telegram_id, is_admin=True)
                
                success_text = f"""✅ *تم إضافة النجوم بنجاح!*

👤 المستخدم: {user.get('first_name', 'غير معروف')}
🆔 المعرف: `{user_telegram_id}`
⭐ النجوم المضافة: {stars_amount}
💰 المعادل: ${usd_equivalent:.2f}
🌟 الرصيد الجديد: {new_balance} نجمة"""
                
                back_keyboard = InlineKeyboardMarkup([
                    [InlineKeyboardButton("⭐ إضافة نجوم لمستخدم آخر", callback_data="add_user_stars")],
                    [InlineKeyboardButton("💰 عرض الأرصدة", callback_data="view_balances")],
                    [InlineKeyboardButton("🔙 العودة لإدارة المحافظ", callback_data="manage_wallet")]
                ])
                await send_admin_message(telegram_id, success_text, back_keyboard)
                
            except ValueError:
                await send_admin_message(telegram_id, "❌ يرجى إدخال عدد صحيح من النجوم:")
